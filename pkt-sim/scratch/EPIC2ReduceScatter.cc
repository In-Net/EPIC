#ifndef EPIC2_REDUCE_SCATTER_H
#define EPIC2_REDUCE_SCATTER_H
...
NS_LOG_COMPONENT_DEFINE("EPIC2ReduceScatterTest");
...
class EPIC2ReduceScatterTag : public Tag{
    public:

        EPIC2ReduceScatterTag() {m_seq = 0; m_idx = 0; m_type = DATA;}

        static TypeId GetTypeId() {
            static TypeId tid = TypeId("ns3::EPIC2ReduceScatterTag")
                .SetParent<Tag>()
                .AddConstructor<EPIC2ReduceScatterTag>();
            return tid;
        }
        ...
        void Print(std::ostream& os) const override {
            os << "EPIC2ReduceScatterTag: seq=" << m_seq 
               << ", idx=" << m_idx 
...
class EPIC2ReduceScatterSenderApp : public Application {
    public:
        EPIC2ReduceScatterSenderApp() : m_rankId(0), m_isRoot(false), m_sentPktNum(0), m_recvPktNum(0) {};
        ~EPIC2ReduceScatterSenderApp(){};
...
void EPIC2ReduceScatterSenderApp::StartApplication() {
    ...
    m_recvSocket->SetRecvCallback(MakeCallback(&EPIC2ReduceScatterSenderApp::RecvPacket, this));
    ...
    Simulator::Schedule(MicroSeconds(1), &EPIC2ReduceScatterSenderApp::StartReduceScatter, this);
}

void EPIC2ReduceScatterSenderApp::StopApplication() {
    if(m_sendSocket) m_sendSocket->Close();
    if(m_recvSocket) m_recvSocket->Close();
}

void EPIC2ReduceScatterSenderApp::InitComm(uint64_t dataSize, uint32_t pktPayloadSize, uint32_t windowSize, 
    std::map<uint32_t, Address> rankAddrMap) {
    m_dataSize = dataSize;
...
void EPIC2ReduceScatterSenderApp::StartReduceScatter() {
    if (m_isRoot) {
...
void EPIC2ReduceScatterSenderApp::RecvPacket(Ptr<Socket> socket) {
    if (m_isRoot) {
...
void EPIC2ReduceScatterSenderApp::RecvAggPacket(Ptr<Socket> socket) {
    Ptr<Packet> pkt;
    while ((pkt = socket->Recv())) {
        ...
        if (tag.GetType() != AGGREGATED) {
            NS_LOG_INFO("Rank " << m_rankId << " Error: receive non-AGGREGATED packet!");
        }
        else {
            m_recvPktNum++;
            ...
            if (m_recvPktNum == m_pktNum) {
                NS_LOG_INFO("[" << Simulator::Now().GetMicroSeconds() << "us]" << " ReduceScatter completed!");
...
void EPIC2ReduceScatterSenderApp::SendAckPacket(uint32_t seq, uint32_t idx) {
...
void EPIC2ReduceScatterSenderApp::RecvAckPacket(Ptr<Socket> socket) {
    Ptr<Packet> pkt;
    while ((pkt = socket->Recv())) {
        EPIC2ReduceScatterTag tag;
        pkt->PeekPacketTag(tag);
        ...
        if (tag.GetType() != ACK) {
            NS_LOG_INFO("Rank " << m_rankId << " Error: receive non-ACK packet!");
        }
        else {
            m_recvPktNum++;
            uint32_t seq = tag.GetSeq();
...
void EPIC2ReduceScatterSenderApp::SendNextOffSetPacket(uint32_t seq, uint32_t idx) {
    Ptr<Packet> packet = Create<Packet>(m_pktPayloadSize);
...
class EPIC2ReduceScatterReceiverApp : public Application {
    public:
        EPIC2ReduceScatterReceiverApp() {}
        ~EPIC2ReduceScatterReceiverApp() {}
...
void EPIC2ReduceScatterReceiverApp::StartApplication() {
    m_recvSocket = Socket::CreateSocket(GetNode(), UdpSocketFactory::GetTypeId());
    m_recvSocket->SetAttribute("RcvBufSize", UintegerValue(1024 * 1024 * 1024)); // 1GB
    m_recvSocket->Bind(InetSocketAddress(Ipv4Address::GetAny(), m_port));
    m_recvSocket->SetRecvCallback(MakeCallback(&EPIC2ReduceScatterReceiverApp::RecvPacket, this));
...
void EPIC2ReduceScatterReceiverApp::StopApplication() {
    if(m_sendSocket) m_sendSocket->Close();
    if(m_recvSocket) m_recvSocket->Close();
}

void EPIC2ReduceScatterReceiverApp::InitComm(std::vector<Address> peerAddrList, Address rootAddr, uint32_t switchPoolSize) {
...
void EPIC2ReduceScatterReceiverApp::RecvPacket(Ptr<Socket> socket) {
    Ptr<Packet> packet;
...
bool EPIC2ReduceScatterReceiverApp::AggregatePacket(uint32_t idx, Ptr<Packet> packet) {
...
void EPIC2ReduceScatterReceiverApp::SendAggPacket(uint32_t idx) {
...
int main(int argc, char *argv[]) {
    LogComponentEnable("EPIC2ReduceScatterTest", LOG_LEVEL_INFO);
    ...
    Ptr<EPIC2ReduceScatterReceiverApp> receiverApp = CreateObject<EPIC2ReduceScatterReceiverApp>();
    ...
    for (uint32_t i = 0; i < numWorkers; i++) {
        Ptr<EPIC2ReduceScatterSenderApp> senderApp = CreateObject<EPIC2ReduceScatterSenderApp>();
    ...
    NS_LOG_INFO("========== INC ReduceScatter Start ==========");

    Simulator::Run();
    Simulator::Destroy();
    
    NS_LOG_INFO("========== INC ReduceScatter End ==========");
