#ifndef EPIC3_REDUCE_SCATTER_APPLICATION_H
#define EPIC3_REDUCE_SCATTER_APPLICATION_H
...
NS_LOG_COMPONENT_DEFINE("EPIC3ReduceScatterTest");
...
class EPIC3ReduceScatterTag : public Tag {
public:
    EPIC3ReduceScatterTag() : m_psn(0), m_idx(0), m_wid(0), m_type(DATA) {}

    static TypeId GetTypeId() {
        static TypeId tid = TypeId("ns3::EPIC3ReduceScatterTag")
            .SetParent<Tag>()
            .AddConstructor<EPIC3ReduceScatterTag>();
        return tid;
    }
    ...
    void Print(std::ostream& os) const override {
        os << "EPIC3ReduceScatterTag: psn=" << m_psn 
           << ", idx=" << m_idx 
           << ", wid=" << m_wid
           << ", type=" << (int)m_type;
    }
...
class EPIC3ReduceScatterSenderApp : public Application {
public:
    EPIC3ReduceScatterSenderApp() : m_rankId(0), m_rootRank(0), m_sentPktNum(0), m_recvPktNum(0), 
                      m_retxTimeout(MicroSeconds(100)) {}
    ~EPIC3ReduceScatterSenderApp() {}
...
void EPIC3ReduceScatterSenderApp::StartApplication() {
    ...
    m_recvSocket->SetRecvCallback(MakeCallback(&EPIC3ReduceScatterSenderApp::RecvPacket, this));
    ...
    Simulator::Schedule(MicroSeconds(1), &EPIC3ReduceScatterSenderApp::StartReduce, this);
}

void EPIC3ReduceScatterSenderApp::StopApplication() {
...
void EPIC3ReduceScatterSenderApp::InitComm(uint64_t dataSize, uint32_t pktPayloadSize, uint32_t windowSize) {
    m_dataSize = dataSize;
...
void EPIC3ReduceScatterSenderApp::StartReduce() {
    if (IsRootRank()) {
...
void EPIC3ReduceScatterSenderApp::RecvPacket(Ptr<Socket> socket) {
    Ptr<Packet> pkt;
    while ((pkt = socket->Recv())) {
        EPIC3ReduceScatterTag tag;
...
void EPIC3ReduceScatterSenderApp::SendDataPacket(uint32_t psn, uint32_t idx) {
    ...
    EventId timerId = Simulator::Schedule(m_retxTimeout, 
                                          &EPIC3ReduceScatterSenderApp::RetransmitData, 
                                          this, idx);
    m_dataTimers[idx] = timerId;
}

void EPIC3ReduceScatterSenderApp::SendAckPacket(uint32_t psn, uint32_t idx) {
...
void EPIC3ReduceScatterSenderApp::RetransmitData(uint32_t idx) {
...
void EPIC3ReduceScatterSenderApp::CancelDataTimer(uint32_t idx) {
...
class EPIC3ReduceScatterReceiverApp : public Application {
public:
    EPIC3ReduceScatterReceiverApp() : m_rankNum(0), m_rootRank(0), m_aggTimeout(MicroSeconds(100)) {}
    ~EPIC3ReduceScatterReceiverApp() {}
...
void EPIC3ReduceScatterReceiverApp::StartApplication() {
    m_recvSocket = Socket::CreateSocket(GetNode(), UdpSocketFactory::GetTypeId());
    m_recvSocket->SetAttribute("RcvBufSize", UintegerValue(1024 * 1024 * 1024));
    m_recvSocket->Bind(InetSocketAddress(Ipv4Address::GetAny(), m_port));
    m_recvSocket->SetRecvCallback(MakeCallback(&EPIC3ReduceScatterReceiverApp::RecvPacket, this));
...
void EPIC3ReduceScatterReceiverApp::StopApplication() {
...
void EPIC3ReduceScatterReceiverApp::InitComm(std::vector<Address> peerAddrList, uint32_t poolSize, uint32_t rootRank) {
...
void EPIC3ReduceScatterReceiverApp::RecvPacket(Ptr<Socket> socket) {
    Ptr<Packet> pkt;
    while ((pkt = socket->Recv())) {
        EPIC3ReduceScatterTag tag;
        pkt->PeekPacketTag(tag);
...
void EPIC3ReduceScatterReceiverApp::HandleDataPacket(uint32_t psn, uint32_t idx, uint32_t wid, Ptr<Packet> pkt) {
...
void EPIC3ReduceScatterReceiverApp::HandleAckPacket(uint32_t psn, uint32_t idx, uint32_t wid) {
    if (wid != m_rootRank) {
        return;
    }
...
void EPIC3ReduceScatterReceiverApp::SendAckToWorker(uint32_t psn, uint32_t idx, uint32_t wid) {
    Ptr<Packet> packet = Create<Packet>(0);
...
void EPIC3ReduceScatterReceiverApp::SendNakToWorkers(uint32_t psn, uint32_t idx, const std::vector<uint32_t>& workers) {
...
void EPIC3ReduceScatterReceiverApp::OnAggregationComplete(uint32_t idx) {
    SendAggregatedToRoot(idx);
    m_aggPool[idx].m_sentToRoot = true;
    
    EventId timerId = Simulator::Schedule(m_aggTimeout,
                                          &EPIC3ReduceScatterReceiverApp::RetransmitToRoot,
                                          this, idx);
    m_aggTimers[idx] = timerId;
}

void EPIC3ReduceScatterReceiverApp::SendAggregatedToRoot(uint32_t idx) {
...
void EPIC3ReduceScatterReceiverApp::RetransmitToRoot(uint32_t idx) {
...
void EPIC3ReduceScatterReceiverApp::CancelAggTimer(uint32_t idx) {
...
int main(int argc, char *argv[]) {
    LogComponentEnable("EPIC3ReduceScatterTest", LOG_LEVEL_INFO);
    ...
    Ptr<EPIC3ReduceScatterReceiverApp> receiverApp = CreateObject<EPIC3ReduceScatterReceiverApp>();
    ...
    for (uint32_t i = 0; i < numWorkers; i++) {
        Ptr<EPIC3ReduceScatterSenderApp> senderApp = CreateObject<EPIC3ReduceScatterSenderApp>();
    ...
    NS_LOG_INFO("========== EPIC3 ReduceScatter Start ==========");

    Simulator::Run();

    NS_LOG_INFO("========== EPIC3 ReduceScatter End ==========");
