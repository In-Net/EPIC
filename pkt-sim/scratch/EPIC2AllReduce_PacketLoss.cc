...
// 对应zzz_savefiles/scratch/INCAllReduce_PacketLoss_plus.cc

enum PacketType : uint8_t {
    DATA = 0,       // 原始数据包（worker → switch）
    AGGREGATED = 1, // 聚合后的包（switch → worker）
    ACK = 2         // ACK包（双向：worker -> switch, switch -> worker）
};
...
private:
    bool DoCorrupt(Ptr<Packet> p) override {
        ...
        // 只丢 DATA 包（不丢 ACK / AGGREGATED 等其它类型）
        if (tag.GetType() != DATA) {
            return false;
        }
        ...
        if (shouldDrop) {
            ...
            std::clog << "[LossModel] t=" << Simulator::Now().GetMicroSeconds() << "us"
                      ...
                      << " type=" << (tag.GetType()==DATA?"DATA":(tag.GetType()==AGGREGATED?"AGG":"ACK"))
...
class INCSenderApp : public Application {
    public:
        INCSenderApp() : m_rankId(0), m_sentPktNum(0), m_recvPktNum(0), m_recvAckNum(0),
                         m_retxTimeout(MicroSeconds(100)) {};  // 100 微秒超时
        ~INCSenderApp(){};
        ...
        void SendAckToSwitch(uint32_t seq, uint32_t idx, uint8_t ver);  // 新增：发送ACK给Switch
        void FillWindow();  // 单一滑动窗口：从右沿(m_nextSeq)补发新包
        
        // 重传相关
        void RetransmitPacket(uint32_t idx);
        void FastRetransmit(uint32_t seq);  // 端侧快重传：失序确认(3 dup-ACK)触发，不等 RTO
        ...
        uint32_t m_recvAckNum;         // 已接收ACK包数（最终判断依据）
        ...
        // 端侧快重传（TCP 3-dup-ACK 等价）：统计当前空洞(m_windowBase)收到的失序确认数
        uint32_t m_dupAckCount;                            // 当前空洞的失序确认计数
};
...
void INCSenderApp::RecvAggPacket(Ptr<Socket> socket) {
    ...
    while ((pkt = socket->Recv())) {
        ...
        if (tag.GetType() == AGGREGATED) {
            ...
            // 发送 ACK 给 Switch（不取消定时器，等收到 Switch 的 ACK 再取消）
            ...
            if (it == m_pendingPkts.end()) {
                NS_LOG_INFO("[Rank " << m_rankId << "] t=" << Simulator::Now().GetMicroSeconds() << "us"
                            << " event=RECV_DUP_ACK seq=" << seq);
                continue;
            }

            NS_LOG_INFO("[Rank " << m_rankId << "] t=" << Simulator::Now().GetMicroSeconds() << "us"
                        << " event=RECV_ACK seq=" << seq
                        << " idx=" << idx << " ver=" << (uint32_t)ver);
            ...
            uint32_t oldBase = m_windowBase;
            while (m_windowBase < m_nextSeq &&
                   m_pendingPkts.find(m_windowBase) == m_pendingPkts.end()) {
                m_windowBase++;
            }

            // 端侧快重传（TCP 3-dup-ACK 等价，交换机保持透明）：
            // 左沿前移 = 空洞被补上 → 计数清零；
            // 左沿没动而本次确认的是更高 seq（m_windowBase 仍未确认）→ 失序确认，计数+1；
            // 累计到 3 个失序确认即立刻重传该空洞，不等 RTO。
            if (m_windowBase > oldBase) {
                m_dupAckCount = 0;
            } else if (seq > m_windowBase &&
                       m_pendingPkts.find(m_windowBase) != m_pendingPkts.end()) {
                m_dupAckCount++;
                if (m_dupAckCount == 3) {
                    FastRetransmit(m_windowBase);
                }
            }
            ...
            if (m_recvAckNum == m_pktNum) {
                NS_LOG_INFO("[Rank " << m_rankId << "] t=" << Simulator::Now().GetMicroSeconds() << "us"
                            ...
                            << " recv_ack=" << m_recvAckNum << " retx=" << m_retxPktNum);
            }
        }
        else {
...
void INCSenderApp::SendAckToSwitch(uint32_t seq, uint32_t idx, uint8_t ver) {
    Ptr<Packet> packet = Create<Packet>(0);  // size = 0 的 ACK 包
    ...
    NS_LOG_INFO("[Rank " << m_rankId << "] t=" << Simulator::Now().GetMicroSeconds() << "us"
                << " event=ACK_SEND dst=Switch seq=" << seq
                << " idx=" << idx << " ver=" << (uint32_t)ver);
}
...
// 端侧快重传：失序确认(3 dup-ACK)触发——取消旧 RTO 定时器并立即重发（复用 RetransmitPacket 逻辑）。
// 注意：这是纯端侧机制，无法定位是哪个 worker 丢的，故所有看到该空洞的 worker 都会重传（冗余仍在，
// 但恢复从“等满 RTO”加速为“3 个失序确认”）。RTO 仍作兜底（应对快重传包再次丢失）。
void INCSenderApp::FastRetransmit(uint32_t seq) {
    ...
    RetransmitPacket(seq);  // 重发 + 重挂 RTO 定时器
}
...
class INCReceiverApp : public Application {
    public:
        ...
        void SendAckToWorker(uint32_t seq, uint32_t idx, uint8_t ver, uint32_t wid);  // 发送ACK给指定worker
...
void INCReceiverApp::RecvAggPacket(Ptr<Socket> socket) {
    ...
    while ((packet = socket->Recv())) {
        ...
        // 处理 ACK 包：Worker 发送的 ACK，需要回复 ACK
        if (tag.GetType() == ACK) {
            NS_LOG_INFO("[Switch] t=" << Simulator::Now().GetMicroSeconds() << "us"
                        << " event=RECV_ACK src=Rank" << wid
                        << " seq=" << seq << " idx=" << idx << " ver=" << (uint32_t)ver);
...
// 发送 ACK 给指定 worker（响应 worker 的 ACK）
void INCReceiverApp::SendAckToWorker(uint32_t seq, uint32_t idx, uint8_t ver, uint32_t wid) {
    Ptr<Packet> packet = Create<Packet>(0);  // size = 0 的 ACK 包
...
int main(int argc, char *argv[]) {
    ...
    // 创建日志文件
    //std::ofstream logFile("INCPlus_simulation_" + std::to_string(packetLossLinkNum) + "linkloss.log");
    ...
    // 写入丢包日志文件
    PacketDropLogger::Instance().WriteToFile("INCPlus_packet_drop.log");
