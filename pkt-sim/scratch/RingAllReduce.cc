...
NS_LOG_COMPONENT_DEFINE("RingAllReduceTest");
...
class RingAllReduceApp : public Application {
...
RingAllReduceApp::RingAllReduceApp()
{
    NS_LOG_FUNCTION(this);
}

void RingAllReduceApp::StartApplication()
{
    NS_LOG_FUNCTION(this);
    ...
    m_recvSocket->SetRecvCallback(MakeCallback(&RingAllReduceApp::RecvChunk, this));

    m_sendSocket = Socket::CreateSocket(GetNode(), UdpSocketFactory::GetTypeId());
    m_sendSocket->Bind();
    m_sendSocket->Connect(m_nextRankAddr);
    Simulator::Schedule(MicroSeconds(1), &RingAllReduceApp::StartAllReduce, this);
}

void RingAllReduceApp::StartAllReduce()
{
    NS_LOG_INFO("[" << Simulator::Now().GetMicroSeconds() << "us] Rank " << m_myRankId << ": start AllReduce!");
    m_step = 1;
    SendChunk(m_myRankId);
}

void RingAllReduceApp::StopApplication()
{
    NS_LOG_FUNCTION(this);
...
void RingAllReduceApp::PrintChunkList()
{
...
void RingAllReduceApp::InitComm(std::vector<uint32_t> nodeIdList, uint32_t myRankId,
                                uint32_t nextRankId, Address nextRankAddr, uint16_t port, 
...
void RingAllReduceApp::SendChunk(uint32_t chunkId)
{
    NS_LOG_FUNCTION(this);
...
void RingAllReduceApp::RecvChunk(Ptr<Socket> socket)
{
    NS_LOG_FUNCTION(this);
    ...
    while ((packet = socket->Recv()))
    {
        ...
        if (m_curChunkRecvBytes >= m_chunkList[chunkId].chunkSize) {
            ...
            m_step++;
            if (m_step >= 2 && m_step <= k) { // Reduce-Scatter
                Simulator::Schedule(MicroSeconds(1), &RingAllReduceApp::ReduceSendChunk, this, chunkId, chunkRankIdList);
                m_curChunkRecvBytes = 0;
                m_curChunkRecvPktNum = 0;
                break;
            }
            else if (m_step >= (k + 1) && m_step <= (2 * k - 2)) { //All-Gather
                ReduceChunk(chunkId, chunkRankIdList);
                Simulator::Schedule(MicroSeconds(1), &RingAllReduceApp::SendChunk, this, chunkId);
                m_curChunkRecvBytes = 0;
                m_curChunkRecvPktNum = 0;
                break;
            }
...
void RingAllReduceApp::ReduceChunk(uint32_t chunkId, uint64_t chunkRankIdList)
{
    NS_LOG_FUNCTION(this);
    m_chunkList[chunkId].chunkRankIdList |= chunkRankIdList;
}

void RingAllReduceApp::ReduceSendChunk(uint32_t chunkId, uint64_t chunkRankIdList)
{
    NS_LOG_FUNCTION(this);
    ReduceChunk(chunkId, chunkRankIdList);
    SendChunk(chunkId);
...
{
    LogComponentEnable("RingAllReduceTest", LOG_LEVEL_INFO);
    ...
    for (uint32_t i = 0; i < numRanks; i++)
    {
        ...
        Ptr<RingAllReduceApp> app = CreateObject<RingAllReduceApp>();
